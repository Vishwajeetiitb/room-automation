  ...
  Plugin.prototype = {
    ...
    initialize: function() {
      ...
      this.domElements = {
        ...
        main: this.element
          .wrap(this.htmlStructure.wrap)
          .parent()
          .addClass(this.options.themeClass),
        switchButton: $(this.htmlStructure.switchButton),
        overlay: $(this.htmlStructure.overlay)
      };

      // Append addional dom elements:
      this.domElements.main.append([
        this.domElements.switchButton,
    ...
    renderMarks: function() {
      ...
      for (; i < len; i += 1) {
        deg += degPerStep;
        var mark = $(this.htmlStructure.mark).css({
          transform:
            "rotate(" +
            deg +
            "deg) translate(0, -" +
            (this.domElements.main.width() / 2 +
              this.domElements.main.width() * 0.1) +
            "px)"
        });
        marks.append(mark);
      }
    ...
    stopHandling: function() {
      $(document)
        .off("mouseup", $.proxy(this.onMouseUp, this))
      ...
      var offset = this.domElements.main.offset(),
        radians = Math.atan2(
          this.mousePosition.x -
            (offset.left + this.domElements.main.width() / 2),
          this.mousePosition.y -
            (offset.top + this.domElements.main.height() / 2)
        );
    ...
    calculateValueByDeg: function() {
      ...
      if (this.switchDeg - this.options.beginDeg > 0) {
        this.valueInPercent =
          (this.switchDeg - this.options.beginDeg) / this.totalDeg;
      } else {
        this.valueInPercent =
          (this.switchDeg - this.options.beginDeg + 360) / this.totalDeg;
      }
      ...
      this.value =
        ~~(
          (range * this.valueInPercent < 0 ? -0.5 : 0.5) +
          (range * this.valueInPercent) / this.options.step
        ) * this.options.step;
      this.value += this.options.minimum;

      if (
        this.options.lengthDeg === 360 &&
        (this.value === this.options.minimum ||
    ...
    rotateSwitch: function(snap) {
      var deg = 0,
        ...
        roundedDeg =
          (this.value / this.steps) * this.totalDeg -
          this.options.minimum * this.degPerStep,
        difference = Math.abs(Math.abs(exactDeg) - Math.abs(roundedDeg)),
        rotateString = "";

      if (
        snap === true ||
        (this.options.snapInMotion === true && difference < this.degPerStep / 6)
      ) {
    ...
    readValueFromInput: function() {
      ...
      this.value -= this.options.minimum;
      this.valueInPercent =
        this.value / (this.options.maximum - this.options.minimum);
    },
